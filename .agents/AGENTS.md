...
- **Never Use `any`:** The `any` type is strictly forbidden. Use `unknown` if the shape of the data is truly not guaranteed, and perform type narrowing/checking before accessing properties. Prefer casting to specific types (e.g., `BatchData` from `src/renderer/src/utils/forms.ts`) instead of `any`.
