...
class Importer:
    ...
    def import_model(
        ...
        if copy:
            path_new = check_dir(self.root / 'models') / Path(path).name #  TODO: use const for model dir
        ...
        self.G.add_node(name, **model.to_dict())
    ...
    def add_external_source(
        ...
        source = ExternalSource(path=source_root)
        self.G.add_node(source_name, **source.to_dict())


    def scan_dir(self, parent_path, parent_node, current_time):
        # Add/update children
        ...
        for idx, child_path in enumerate(parent_path.iterdir()):
            if child_path.is_dir():
                ...
                if not self.G.has_node(child_path.name):
                    ...
                    self.G.add_node(
                        child_path.name, **audio_set.to_dict(), parent=parent_node
                    )
                ...
                if not self.G.has_node(child_path.stem):
                    ...
                    self.G.add_node(child_path.stem, **audio.to_dict())
        ...
        for audio_set_dir in source_root.iterdir():
            if audio_set_dir.is_dir():
                ...
                if not self.G.has_node(audio_set_dir.name):
                    audio_set = Set(alias=set_name, created=current_time)
                    self.G.add_node(set_name, **audio_set.to_dict())
                    self.G.add_edge(
    ...
    def import_audio_set(
        ...
        set_dir_new = check_dir(self.root / 'audio' / source_name / set_name) # TODO: use const for audio dir
        for idx, sample_path in enumerate(set_dir.iterdir()):
            if sample_path.suffix == '.mp3':
                ...
                self.G.add_node(sample_path.stem, **audio.to_dict())
