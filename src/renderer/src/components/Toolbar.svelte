<script lang="ts">
  ...
  async function handleAddExternalSource(): Promise<void> {
    try {
      const sourcePath = await window.api.selectDirectory()
      if (sourcePath) {
        onaddExternalSource?.(sourcePath)
      }
    } catch (error) {
      console.error('Failed to select external source directory:', error)
    }
  }
